...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (postToFeedBtn) {
        postToFeedBtn.addEventListener('click', async function() {
            await saveWorkout('public', false); // false = not saved to profile
        });
    }
    ...
    if (saveToProfileBtn) {
        saveToProfileBtn.addEventListener('click', async function() {
            ...
            await saveWorkout('private', true); // true = saved to profile
        });
    }
});
...
async function saveWorkout(privacy, savedToProfile = false) {
    ...
    try {
        ...
        const result = await apiCall('/log-workout', 'POST', {
            ...
            saved_to_profile: savedToProfile  // Flag to indicate this was saved to profile
        });
